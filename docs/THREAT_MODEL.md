...
| T4 | API abuse / cost exhaustion | Best-effort per-isolate sliding-window rate limit (5/min/IP), concurrency cap (4), optional Turnstile | Isolate-local limiter resets; durable limiter is V0.1 |
| T5 | Secret leakage to browser | Provider secrets are Worker bindings only; the client bundle contains no provider keys; `_headers` CSP restricts sources | Operator misconfiguration |
| T6 | Prompt injection into AI explainer | AI receives structured JSON of deterministic text only; output is plain text appended as the client explanation; AI cannot create findings, severities, evidence or numbers; AI failures fall back | Rephrasing could still distort nuance → covered by claims-guard tests on deterministic text only; AI output is explicitly optional and off by default |
| T7 | XSS via scanned content | No scanned HTML is rendered; only extracted text; React escapes; CSP `script-src 'self'` | Very low |
| T8 | Report tampering | Every finding references evidence ids; evidence carries provenance, sha-256 of bodies where captured, and `recordedAt` for fixtures; audits are deterministic for identical inputs | No cryptographic signing in V0 (Ed25519-style bundle signing could be imported from `argus` later) |
| T9 | False legal claims | Wording rules + machine-enforced claims guard test over all reports; privacy findings are labelled indicators | Human review still required |
| T10 | Hostile/huge responses | Timeout (10 s), body cap + truncation flag, content-type checks, no rendering | Slow sites lengthen audits (bounded by budget) |
...
Run before any public deployment (all items verified during the V0 build where
marked done):
...
- [x] No secrets, tokens or `.dev.vars` committed; `.gitignore` covers them
- [x] CSP + security headers defined for static assets (`apps/web/public/_headers`) and API responses
...
- [x] Report provenance: every finding → evidence ids → raw data
- [ ] Mobile layout and keyboard accessibility re-verified on the deployed build
- [ ] Clean-clone reproduction on the CI runner (the workflow does this on push)
- [ ] Turnstile secret configured (if deployed publicly) and verified
- [ ] Operator-facing authorisation note shown in the UI (already present in the form)
